...
public class RoundedImageView extends AppCompatImageView {
    ...
    private int mRebound;  // 回弹值
    private int mDisappear;  // 消失值
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mListener != null) {
                    if (getLeft() < -mDisappear) {
                        mListener.onSwiped();
                    } else {
                        mListener.onSwiping(left);
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                if (getLeft() < -mRebound) {
                    this.animate()
                            .translationXBy(-mDisappear - getLeft())
                            .setDuration(150)
                    ...
                    new CountDownTimer(150, 10) {
                        ...
                        public void onTick(long millisUntilFinished) {
                            int index = (int) (millisUntilFinished / 10);
                            int l = tx * index;
                            layout(l, getTop(), l + getWidth(), getBottom());
                            if (mListener != null)
                                mListener.onSwiping(l);
                        }
